...
habitController.addHabit = async (req, res, next) => {
  ...
  const inDB = await db.User.findOne({
    email,
    "habit.name": habit
  });
  console.log("if already in db ", inDB);
  if (inDB) return next({
    err: "habit is already in the db for that user"
  });
  ...
  try {
    ...
    return next({
      err: "error with updating the habit: " + e
    });
  }
  return next();
};
...
habitController.removeHabit = async (req, res, next) => {
  ...
  try {
    ...
    return next({
      err: "error with removing habit: " + e
    });
  }
};
